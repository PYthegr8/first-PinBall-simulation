...
# CS 151S
# Fall 2015
#updated by Papa Yaw Owusu Nti(2023)
# Project 9
# Collision handler
#
...
# Create a dictionary to store collision functions based on pairs of objects.
collision_router = {}

# Assign collision functions to specific pairs of object types.
...
collision_router[('ball', 'pentagon')] = collision_ball_ball
collision_router[('ball', 'customshape')] = collision_ball_ball
collision_router[('ball', 'floor')] = collision_ball_floor
collision_router[('ball', 'wall')] = collision_ball_wall
collision_router[('customshape', 'ball')] = collision_ball_ball
collision_router[('customshape', 'block')] = collision_ball_block
collision_router[('customshape', 'triangle')] = collision_ball_ball
collision_router[('customshape', 'pentagon')] = collision_ball_ball

# Define a generic collision function that uses the collision_router dictionary.
def collision(ball, thing, dt):
    # Create a key based on the types of the two objects involved in the collision.
    key = (ball.getType(), thing.getType())

    # Check if a specific collision function is available for the given object types.
    if key in collision_router:
        # Call the appropriate collision function and return the result.
        return collision_router[key](ball, thing, dt)
    else:
        # Print a message indicating that no collision function is available for the object types.
        print(f"No function for: {key}")
        return False
